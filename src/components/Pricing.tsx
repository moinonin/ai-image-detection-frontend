...
const Pricing: React.FC = () => {
  ...
  const handleSubscribe = async (product: PolarProduct) => {
    ...
    try {
      console.log(`Getting checkout URL for product: ${product.id}`);
      
      // Use the GET endpoint instead
      ...
      if (sessionData.checkout_url) {
        window.open(sessionData.checkout_url, '_blank', 'noopener,noreferrer');
      } else {
      ...
      // Fallback to direct product checkout URL
      setTimeout(() => {
